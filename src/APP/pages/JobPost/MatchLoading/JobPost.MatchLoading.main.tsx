  ...
  useEffect(() => {
    const handleRecommend = async () => {
      ...
      try {
        ...
        const responses: AxiosResponse<RecommendationResponse>[] =
          await Promise.all(
            seniors.map((senior) =>
        ...
        // AxiosResponse에서 data 프로퍼티만 추출
        const dataResponses: RecommendationResponse[] = responses.map(
          (res) => res.data
        );

        // 모든 요청이 성공했는지 확인
        const allSuccessful = dataResponses.every((res) => res.isSuccess);

        if (allSuccessful) {
          // seniors 상태 업데이트 (추천 결과 반영)
          const updatedSeniors = seniors.map((senior, index) => ({
            ...senior,
            recommendations: dataResponses[index].result, // API 결과 저장
          }));
